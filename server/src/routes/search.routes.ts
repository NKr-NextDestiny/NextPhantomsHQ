...
// Global search across trainings, scrims, strats, matches, users, wiki
searchRouter.get("/", authenticate, teamContext, async (req, res, next) => {
  try {
    const q = (req.query.q as string || "").trim();
    if (q.length < 2) {
      res.json({ success: true, data: { trainings: [], scrims: [], strats: [], matches: [], users: [], wiki: [] } });
      return;
    }
    ...
    const [trainings, scrims, strats, matches, users, wiki] = await Promise.all([
      ...
      prisma.scrim.findMany({
        where: { teamId, OR: [{ opponent: contains }, { notes: contains }] },
      ...
      prisma.match.findMany({
        ...
        select: { id: true, opponent: true, map: true, result: true, date: true },
        take: 10,
        orderBy: { date: "desc" },
      }),
